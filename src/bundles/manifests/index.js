...
export const MODEL_BUNDLES = {
  ...
  // `tools/gpu/fold-opendde.js` is that driver; every stage inside it is the
  ...
  // src/af3/weights/diffusion-weights.js.
  //
  ...
  // `src/af3/trunk/template-webgpu.js`.
  //
  ...
  intellifold2: {
    ...
    remote: "https://huggingface.co/sokrypton/localfold/resolve/6634c1398a781455e5be3b06573179f896ae8ad8/intellifold2-int5/",
  ...
  rosettafold3: {
    ...
    remote: "https://huggingface.co/sokrypton/localfold/resolve/6634c1398a781455e5be3b06573179f896ae8ad8/rosettafold3-int5/",
 ...
 * parametrisation - see src/af3/confidence/opendde-confidence.js - and it is ported, so a
...
/**
 * What each family is CALLED, which is provenance rather than decoration.
 *
 * 🔴 ONE TABLE, BECAUSE TWO BUNDLES RUN ONE GRAPH. A fold says which model
 * made it - the status line, the archive, the prediction's `model` field -
 * and an OpenBind fold labelled "AlphaFold 3" is a record of the wrong
 * provenance, which is the whole reason the licence dialog exists. It lived
 * as a private const in web/app.js, so the Colab runtime - which folds in a
 * process with no page - would have had to write a second copy of it.
 */
