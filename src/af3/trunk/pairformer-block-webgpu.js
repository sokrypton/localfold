 ...
 * src/af3/trunk/transition-webgpu.js; at AF3's 128 the split is 1.8% of a trunk for
 ...
 * `pairChannels` and `singleChannels` because `src/af3/weights/weights.js` derives them
...
export class Af3PairformerStackGpu {
  ...
  async #runStack(state, blocks, dialect, options = {}) {
    ...
    // without complaint - see src/af3/weights/weights.js. They go in the shader cache
    ...
    if (!(pairChannels > 0)) {
      throw new Error("pairformer blocks carry no pairChannels; they are built "
        + "by src/af3/weights/weights.js, which derives it from the weights");
    }
    ...
    try {
      ...
      for (let index = 0; index < blocks.length; index += 1) {
        ...
        // This is AF2's idiom, from src/af2/evoformer/stack.js, and it is here for
  ...
  async #encodeBlock(context) {
    ...
    // see src/af3/weights/pair-track-device-weights.js for the table of mappings.
    ...
    // src/af3/weights/device-weights.js - it decodes on the GPU instead, bit for bit,
