...
// 🔴 ONE DEFINITION, IN src/runtime/execution.js - see the note there for the
// four copies this replaced and the one that bound them backwards. Re-exported
// so this module's three callers (add-pair, add-msa, add-single) are unchanged.
import { createAddShader } from "../../runtime/execution.js";
// 🔴 IMPORTED *AND* RE-EXPORTED, not `export ... from`. This module CALLS it
// too (add-pair, add-msa, add-single), and a bare re-export puts the name in
// the module's exports without putting it in its SCOPE - which
// test/module-references.test.js caught immediately.
...
export async function compilePairTrack(cache, options) {
  ...
  const shape = {
    ...
    // in src/kernels/triangle/shaders.js for why it is applied at the centre norm.
  ...
  // src/kernels/triangle/project-matrix.js.
  ...
  for (const direction of ["outgoing", "incoming"]) {
    ...
    // constant repeated in both places did once - see src/kernels/triangle/shaders.js.
    ...
    for (const [name, source] of Object.entries(sources)) {
      ...
      // CLAUDE.md lists. src/af2/evoformer/block.js learned this at the same seam.
  ...
  for (const [key, attention, transpose] of (gridAttention
    ...
    // src/af3/trunk/grid-project-matrix.js.
...
export function packPairTrackWeights(block, channels = PAIR_CHANNELS, weightPrecision = "f32",
  ...
  // src/af3/weights/pair-track-device-weights.js.
...
export function encodePairTrack(context) {
  ...
  for (const direction of ["outgoing", "incoming"]) {
    ...
    if (pipelines.projectMatrix !== undefined) {
      ...
      // and the dispatch change. See src/kernels/triangle/project-matrix.js.
  ...
  for (const [key, w] of ((context.gridAttention ?? true)
    ...
    // See the note on the attend kernel and src/af3/trunk/grid-attention-matrix.js.
    ...
    if (pipelines.gridProjectMatrix !== undefined) {
      ...
      // the second one - see sourceModulate in src/kernels/matrix-linear.js.
