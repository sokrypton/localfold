...
// helper lives there.
import { tileThatFits } from "../../kernels/triangle/shaders.js";
/**
...
export function createGridAttentionShaders(
  ...
  // than assumed - src/kernels/attention.js records a tile in AF2 that got
  ...
  // sweep records in src/kernels/attention.js. `projectOutRows` is a separate
...
${overRows((r) => `  if (first + ${r}u < PAIRS) {
  ...
  // src/kernels/attention.js, where the staged reads priced out at 8.7 ms of
...
export class Af3GridSelfAttentionGpu {
  ...
  async run(pair, mask, shape, weights, dialect, options = {}) {
    ...
    // the default one, or absent. See src/af3/trunk/grid-attention-matrix.js.
