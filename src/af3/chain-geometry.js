/**
 * The gate that says a fold is a chain - shared by every fold tool AND by the
 * page.
 *
 ...
 *
 * 🔴 AND IT LIVED UNDER `tools/`, WHERE THE PAGE COULD NOT REACH IT - so the
 * SITE ran no geometry check at all, for any model. Every CLI fold gates on
 * this and the one path a visitor actually takes did not, which is the same
 * shape as `LOCALFOLD_STOCK_FLAGS`: the configuration every gate checks was not
 * the one that ships. Measured: intellifold2 in flow mode returns a fold this
 * rule REFUSES on 1 seed in 6 - CA median 4.255 A against 3.80 - with pLDDT
 * 83.30 saying nothing is wrong, and the page would have drawn it.
 *
 * The message is built by the CALLER's wrapper, because "pass
 * --allow-broken-geometry" is advice for a terminal and not for a browser.
 */
