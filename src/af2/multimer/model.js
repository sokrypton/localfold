 ...
 *
 * See src/af2/multimer/input-embedder.js for why this is a copy. It differs from
 * src/af2/model/monomer.js in three imports and two options - the evoformer block
 * and pair embedder come from src/af2/multimer/, the structure module reaches the
...
export class AlphaFoldUnifiedGpu {
  ...
  async predict(featuresByRecycle, weights,
    ...
    try {
      ...
      // in src/af2/model/monomer.js. 348 MB of JavaScript zeros at 825 residues.
      ...
      let previousMsa = resume?.msa === undefined
        ? carried("multimer.recycle-msa-zero", length * 256)
        : execution.upload("multimer.recycle-msa", resume.msa, carry);
      let previousPair = resume?.pair === undefined
        ? carried("multimer.recycle-pair-zero", length * length * 128)
        : execution.upload("multimer.recycle-pair", resume.pair, carry);
    ...
    const capturePair = async(name, tensor) => {
      ...
      for (let recycle = firstRecycle; recycle < featuresByRecycle.length; recycle += 1) {
        ...
        // see the same block in src/af2/model/monomer.js for what it is worth.
        if (recycle === 0) {
      ...
      // folds never read. See the note in src/af2/model/monomer.js.
