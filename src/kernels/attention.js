...
import { ATTENTION_SUBGROUP_SIZE, allowsAttentionSubgroupSize }
 ...
 * four. AF3's triangle projection is the same change; see src/kernels/triangle/
 * shaders.js.
 *
...
fn store4(v: vec4<f32>) -> vec2<u32> {
  ...
  // see src/af3/trunk/grid-attention-webgpu.js.
  //
...
// Re-exported from src/kernels/attention-geometry.js, which is where the
// subgroup-size question went so `supportsAttentionMatrix` could ask it
// without runtime depending on evoformer. Kept on this module's surface
// because probe-kernel.js and attention-subgroup-size.test.js name it here.
export { ATTENTION_SUBGROUP_SIZE, allowsAttentionSubgroupSize };
...
export function selectAttentionFlashKernel(
  ...
  // src/kernels/attention-matrix.js: the units contract sixteen queries
 ...
 * src/kernels/transition.js: contiguous columns per lane would make the
...
export class AttentionGpu {
  ...
  async run(input) {
    ...
    // CLASS NEVER DID. src/af2/evoformer/block.js runs the projected tensors packed
