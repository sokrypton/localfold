...
export function readTensorRange(record, buffer, byteOffset, first, count, copy = false,
  ...
  // 🔴 AND int5 AT ANY GROUP BUT 32 COMES HERE TOO, rather than to the refusal
  // below. The unrolled path is int5-at-32 and nothing else; the refusal was
  // right while it was the only int5 reader, and the general loop beside it has
  // always been able to read the others. `check-quantised-upload.js --codecs=5:64`
  // holds this against the DEVICE decoder at zero differing elements, which is
  // the comparison that caught the 20-byte stride in the first place.
  if (bits !== null && (bits !== 5 || record.block !== 32)) {
    const { block } = record;
  ...
  if (record.dtype === "int5") {
    ...
    if (block !== 32) {
      // Unreachable: the general path above now takes every int5 group but 32.
      // Kept because a reader arriving at the unrolled loop with the wrong
      // group decodes a finite, plausible, WRONG tensor, and a guard that
      // cannot fire costs nothing beside that.
      throw new Error(`int5 at group ${block} reached the 20-byte unrolled path`);
    }
    const groups = Math.ceil(elements / block);
