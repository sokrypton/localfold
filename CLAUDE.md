...
| ...and 9 knobs it CANNOT reach | `--tune` splits its argument on commas, so an object- or tile-valued knob cannot be written on a command line: `atomRowTile`, `attentionMatrixTile`, `diffusionSplitK`, `diffusionTokenTile`, `gridAttendMatrixTile`, `halfPrecision`, `matrixLinear`, `stagedMatrixBlock`, `trianglePairProjectTile`. `fold.js --split-k=<json>` exists for one of them; the other eight are unaudited |
