...
| **Does OPENDDE's trunk and confidence match it?** | `tools/gpu/fold-opendde.js --trunk-oracle= --dump=` and `tools/gpu/check-opendde-confidence-oracle.js` - both new, and OpenDDE was the last family with neither. Its confidence needed its own dumper (`dump_af3_opendde_confidence.py`): `dump_af3_confidence.py` indexes a tensor OpenDDE does not have and dies with a KeyError. The confidence head WAS wrong (PAE 4.68e-3) and is fixed in 8dbb8cd - **8.46e-7 on the A100, 9.11e-7 on the M2**; its atom encoder is exact (0 to 1.79e-7, masked). 🔴 **Its trunk is exact into the pairformer (target_feat 3.14e-8) and STILL WRONG from the template stage on (2.07e-2, inherited to 1.08e-1)** on a query with NO template, seen on both machines. That is the one open defect |
...
| **WHICH parts of WHICH model are not exact?** | the sweep in docs/AF3.md: all five trunks on their own reference batch. **Every atom encoder is exact (`target_feat` 1e-8 across the board).** Three are not: OpenDDE's outer product mean **6.01e-2**, protenix2's template stage **3.89e-3** (measured for the first time; its own defect, not OpenDDE's - it already has the gap convention), and AlphaFold 3's own `z_init_generic` **2.20e-4**, four orders worse than every other family's. All three open |
...
| 🔴 **Would a device at WebGPU'S GUARANTEED MINIMUM refuse a kernel?** | `node tools/check-portable-limits.mjs --spec-floor` - and **FOUR OF SIX MODELS DO NOT FOLD**. AlphaFold 3, boltz2 and protenix2 ask for `workgroup_size(512)` and OpenDDE for 384; the standard promises **256** invocations and a 256 X extent. `PORTABLE_CEILINGS` caps at 1024, which is what an A100 and an M2 report - **it is the weakest machine anyone here has MEASURED, not the weakest a conforming browser may be**, so `npm run test:portable` passes all six and cannot see this. 🔴 **AND THE FIRST BLOCKER IS FIXED: the 512 was a PERFORMANCE pick.** `transitionWidth` chooses between 512 and 256 on `transitionThreadTarget`, and it now skips any candidate the device will not run - not a fallback, the narrower width is one of the two the tuner was already choosing between, and on every real part (A100 and M2 both report 1024) nothing changes. **The next blocker is workgroup STORAGE**: "a split transition stages 32768 B, over the limit", 16 KiB being the spec's guarantee against the 32 KiB `PORTABLE_CEILINGS` assumes. `splitTransitionConfig` checks the tuning, the channel width and the matrix capability and NOT the storage its geometry needs - where `resolveGridAttendMatrix` beside it does exactly that and "resolves to false rather than throwing". That is the next piece and it is unwritten. Only AF2 and ESMFold2 fold at the floor today, and AF2's checksum MOVES there (-1294937 against -1287025) because 16 KiB picks a different tile |
...
| **Do the page and the CLI fold the SAME fold?** | they build the batch through one function now - `af3BatchFromA3m` in src/af3/batch.js - and the four families agree on 6MRR with a 128-row search: af3 90.9/90.9, openbind0 88.1/88.1, boltz2 96.1/96.1, protenix2 92.5/92.5, opendde 94.6/94.609 - each arm on the SAME bundle as the other, which openbind0 needed two tries to arrange. 🔴 Before it they did not, and the divergence hid a page-only bug for a day. See docs/AF3.md |
