...
| 🔴 **Does a viewer still show the LAST model's answer under the model you just picked?** | `npm run test:pending` (`tools/check-model-pending.py`). It did: the structure, the contact map and the confidence card look identical whichever row is selected, so switching AlphaFold 3 to Boltz-2 and reading the picture is reading the wrong model's answer with the right model's name over it. `shownFamily` + `syncPendingResult` veil the result boxes with an opaque panel naming the model the page is now set to, and take the numbers away (a stale pLDDT reads as a measurement); switching BACK unveils, because the answer really is that model's. 🔴 **ALL THREE ROWS MOVE THE FAMILY** - the model select, the AF2 number and the PLM row, which `chosenFamily` reads - so a veil on one is two rows switching in silence. 🔴 **AND `undefined` CLAIMS NOTHING**: a dropped file or a restored session is not a fold of ours, and veiling it on load would be the same fault by another route. 🔴 **AND THE DOWNLOAD ROW MAKES THE SAME CLAIM ONE CONTROL ALONG**: both handlers read `activePrediction()` and return in SILENCE when there is none, so the row offered files that did not exist before the first fold and the covered model's fold after a switch. `syncDownloads` is the one rule - the structure and the archive follow the FOLD, the Session button follows what is DRAWN (py2Dmol's session is the viewer's, which a dropped file fills too), a disabled button says why and takes its title back. 🔴 **THE GATE MEASURES PIXELS, NOT A CLASS** - a class with no rule behind it is a viewer still showing the wrong answer - and borrows tools/colab_backend.py as a FIXTURE, because a prediction cannot otherwise be put on a page here without weights. Two mutations: the row changing in silence, and the stylesheet's rule removed (the class arm stays green, which is why the pixels are read). See docs/WEB.md |
| 🔴 **Does the Colab bridge carry a fold BOTH WAYS, and is the feed LIVE?** | `npm run test:colab` (`tools/check-colab-bridge.py`) - **no GPU and no weights**: it starts `tools/colab_backend.py`, which opens `index.html?role=runtime` headlessly, and drives every route from a reader's side over plain HTTP. 🔴 **THE FEED USED TO BE PULLED AND THAT IS WHY IT WAS NOT LIVE.** The backend collected the page's status writes, bar fractions and sampler frames by evaluating a splice over CDP every 250 ms, so a reader saw the fold only as often as a busy page answered the debugger - reported from a real runtime as the bar **sitting at "embedder · 1%" for a whole fold**, everything arriving at the end. `web/colab-bridge.js` **pushes** now, in the task that made the event, and the reader's commands come back through the broker's second mailbox (`/in` and `/out` beside `/up` and `/down`) - so the backend no longer clicks `#predict`, scrapes the status line for the word "failed" or reads the structure out of the download button: CDP is left with starting the browser and naming the card. 🔴 **AND "ONE REQUEST AT A TIME" PUT THE FAULT BACK, WHICH THIS GATE CAUGHT ON ITS FIRST RUN**: holding the next batch until the last RESOLVED needs the main thread to run the response, and a folding page does not give it up - 20 events across 6 s of 300 ms tasks arrived at **p50 3,002 ms, worst 5,702**, against **1 ms and 2** started-and-not-awaited. The cost is ordering, so every event carries the page's `seq` and the reader sorts each batch by it. What the gate proves: the announcement, a `ping` answered as a `pong` (202-414 ms), both clocks on every event (`at` from the page, `got` from the broker, so a slow feed and a slow fold are two numbers), the watermark's idempotence, the 429, a token refusal on all five routes - plus two arms no curl can stand in for: **the READER'S OWN PAGE** in a second browser (weights blocked on the runtime with `Network.setBlockedURLs`, so a real fold fails in seconds instead of pulling hundreds of megabytes; its `Failed to fetch` is what the reader's status line ends up reading) and **the feed under load**, bounded at a second. 🔴 **A FRESH PROFILE HAS ACCEPTED NO MODEL TERMS**, so the reader arm's first run clicked Fold into a terms dialog and sent nothing - a page with a sequence, an enabled button and a status line still reading "Ready.". 🔴 **AND A PREDICTION CROSSES AS JSON, WHICH HAS NO TYPED ARRAYS**: the replacer flattened every Float32Array to a plain array, which looks right until something SLICES one - `download-all` reaches `matrixRows`, which cuts the PAE with `values.subarray(...)`, and a remote fold's download died on *"values.subarray is not a function"* with a perfect structure beside it. The kind travels (`{__typed, v}`) and `revivePrediction` puts it back; the gate asserts the CONSTRUCTOR NAME, because a plain array of the right numbers passes everything else. 🔴 **AND DISCONNECT WALKED AWAY RATHER THAN STOPPING ANYTHING** - the browser on that machine holds the card for as long as it lives. It posts `shutdown`, the broker's own op: the service stops, its browser is terminated, the process exits and the cell ends. 🔴 **AND IT RELEASES THE MACHINE, WHICH I FIRST WROTE DOWN AS IMPOSSIBLE**: `google.colab.runtime.unassign()` is a POST to `http://$TBE_RUNTIME_ADDR/unassign`, a plain address in the environment rather than a call over the kernel's channel, so the broker makes it itself - browser first, machine second, because unassigning pulls the VM out from under whatever had not happened yet. Absent that variable there is no machine to hand back and the button says so instead. The gate stands a STUB in for Colab's runtime service and asserts the one request. After it the page is a VIEWER - `retireFolding` disables Fold and every control that shapes the next one, each saying why, while the viewer, plots and downloads stay - and it does NOT reload, because the server it was served by is what stopped. 🔴 **AND THE BADGE SITS IN ITS OWN SLOT** between the title and the actions: appended to a `space-between` head it pushed Fold out of the place a reader had learnt. Its extra row on a narrow window then broke a PROBE - viewport-rectangle screenshots put the structure box below the fold and both came back identical, reported as "nothing was drawn" - so the veil's shots are page coordinates with `captureBeyondViewport` now. 🔴 **AND THE READER'S PAGE SAYS WHERE FOLD RUNS**: a badge in the page head naming the runtime and its card (`nvidia turing` is the GPU, `google swiftshader` the CPU in its clothes), amber when that runtime stops answering - at the same 20 s the fold loop gives up at, or the page says two things at once - and **Disconnect**, which stops the fold there first and returns this page to folding here. `/health` is asked until it answers ONCE (it reaches over CDP), never polled. Its arms live in `npm run test:pending`, which also CLICKS the PDB download in Colab mode - 478 bytes, 6 atoms - because a button offered over a prediction the runtime never filled looks identical from the outside. 🔴 **AND A RECYCLED RUNTIME IS THE NINTH ARM**: the busy flag is raised by the broker and lowered by the PAGE, so a runtime that died mid-fold took it with it - reader polling for ever, every later fold 429. The page's command poll is the heartbeat (`runtimeSeen`), the reader gives up past 20 s, and an announcement clears the flag; measured 212 ms fresh, 3,341 away, 135 back. 🔴 **AND `threading.Lock` IS NOT REENTRANT** - `_seen()` called inside a held `MAIL_LOCK` hung the whole broker and the gate failed three arms downstream in an innocent route. 🔴 **AND A RELOADED RUNTIME PAGE REPLAYED THE WHOLE SESSION** - it polled `/out` from zero, so the heartbeat arm's navigate-away-and-back made it re-run a fold from ten minutes earlier: **681 MB of weights fetched and a real 58-residue fold on a laptop**, which then filled the viewer the next arm was reading and made its mutation PASS. A page that has just loaded starts at the queue's head now, the frames arm names the structure it pushed (three frames of exactly **four** positions, not "three or more"), and the weights stay blocked for the whole run - nothing in this gate may ever fold. Seven mutations caught: the push removed (four arms red), the arrival stamp dropped, the busy refusal removed, the reader's command never sent, nothing attaches mid-fold, frames collected but not drawn, and a reloaded page replaying. 🔴 **IT CANNOT COVER A FOLD** - the model never runs, and what the reader ingests is four alpha carbons the gate wrote down. See docs/WEB.md |
...
🔴 **ROUND SIX IS `docs/M2-HANDOFF.md`, ON BRANCH `opus-55-opt`**: 65 commits of
speed, memory and cold-run work measured only on the A100 under stock flags -
none of it on the f16 path an M2 runs stock. Start there.
...
| `docs/DAWN.md` | folding in a PROCESS over Dawn instead of a headless Chrome: tried on the `dawn` branch, measured, and **not taken** - what it ports is one protein chain, and a DNA chain came back as a peptide. Also the three findings worth keeping from it: `webgpu@0.6.0`'s f16 aborting the sampler, the 65536 ns `timestamp_quantization` grid that flattened every small kernel's measurement, and a 300 ms poll starving a weight download on two CPUs |
...
| `docs/M2-HANDOFF.md` | 🔴 **ROUND SIX FOR THE M2**: what branch `opus-55-opt` changed, which of it is a default on every device, the knob that is each change's control arm, and the questions only an Apple part can answer (the f16 arms, unified-memory thresholds, occupancy choices on ~10 cores, the cold run on Apple's compiler) |
