...
| `docs/ESMFOLD2_PTM.md` | 🔴 **A SECOND BRIEF FOR THE af3-any-model SIDE, THE SAME SHAPE AS THE BOLTZ2 ONE** - the vendor `esm` package places a phosphoserine at **0.997** where af3-any-model's `esmfold2_lm600m` reads 1.542 (fp32; 1.548 int8, five samples 1.47-1.83) and this port 2.349, all against the same 171-bond control at ~1.00. Not the quantisation, not one sample, not the step budget. Carries what is already excluded on this side (conformer exact, one `refSpaceUid`, bonds present and consumed, `molType` PROTEIN and CORRECT, a ligand in the same fold at 0.958), the discriminator that would halve the search, and the two traps it cost - a near-miss 1.101 from atom names that kept a leaving atom, and a sorted-distance match that is not a shape match |
