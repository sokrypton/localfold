...
| 🔴 **What is a fold CALLED - and is it called that everywhere?** | **`foldStem` in web/app.js: the name box, then a pasted FASTA header, then the model prefix.** It was two identities: a reader could type `calmodulin_4calcium` and get `af3_1` in the object picker, `af3_1.pdb` from the download button and `af3_1_*` throughout the archive, with the typed name in ONE field of ONE file. All three fold paths go through the one resolver now and `buildFoldArchive` lost its `jobName` parameter - the request's name IS the stem. `untitled` means unnamed, so an unnamed fold stays `af3_1`: treating it as a name would make every fold `untitled`, `untitled_2` in the picker, and comparing models is what that picker is for. 🔴 **AND A RENAME HAD TO BREAK A CONTINUATION**: pressing Fold again with nothing changed reuses the trunk and rewinds the object it has, so renaming did NOTHING to the name - `continuedStem` opens a new object on an EXPLICIT rename only, because comparing the resolved name would abolish the continuation for everyone who never names anything (the fallback carries `predictionCount`). 🔴 **AND THE PROBE THAT FOUND IT WAS RACING**: it waited for the status to match /pLDDT/, which the PREVIOUS fold's status already did, so it archived the old prediction and reported the feature broken - **a fold is finished when its object exists**, and the wait must fit cdp.py's 60 s socket timeout. See docs/WEB.md |
