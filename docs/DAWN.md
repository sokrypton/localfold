# Folding in a process over Dawn: tried, measured, and NOT taken

The Colab runtime is a headless Chrome opening `index.html?role=runtime` and
driving its own controls. It was replaced by a node process over Dawn, which
folded - and the replacement was abandoned, because what it ports is **one
protein chain**. The table near the bottom is why, and it is the part of this
document that matters.

This is the record so nobody pays for it twice. The work lives on the `dawn`
branch (22 commits from `20b065d`, unmerged); what came back to `main` is
three things that were never about node - the notebook cleanup, the
`timestamp_quantization` measurement fix, and this file.

🔴 **AND ONE OF ITS FAULTS WAS SILENT, WHICH IS THE REASON TO KEEP READING.**
A DNA chain folded as a twenty-residue PEPTIDE with a plausible confidence,
because `chainKinds` never reached the runtime - `ACGT` being a valid protein
and a valid nucleic chain is a trap this repository already documents. A
refusal is cheap; an answer that is wrong and confident is not.
